...
type readerMy struct {
	words *[]recordMy
}

type recordMy struct {
	word    []byte
	counter int
}
...
func (r *readerMy) containsMy(element []byte) (bool, int) {
	for index, v := range *r.words {
		if bytes.Equal(v.word, element) {
			return true, index
		}
...
func (r *readerMy) read_from_chan(ch chan []byte) {
	for node := range ch {
		state, index := r.containsMy(node)
		if state {
			(*r.words)[index].counter++
		} else {
			record := recordMy{node, 1}
			*r.words = append(*r.words, record)
		}
	}
}
...
func (r *readerMy) get20mostfrequentwordsMy() {
